...
void arch::mp::boot_aps() {
	log.info("Bringing up other APs - DISABLED!");
	...
	//  	bootstrap_struct.compat_gdtr_offset = (uintptr_t)data_page.get() + offsetof(CpuBootstrapPage,
	//  compat_mode_gdt); 	bootstrap_struct.long_gdtr_offset = (uintptr_t)data_page.get() + offsetof(CpuBootstrapPage,
	//  long_mode_gdt); 	bootstrap_struct.cr3 = (uintptr_t)idle_task->parent()->vmm().pml4().get();
