...
class IdeDevice {
	...
	const DriveSelect m_which { DriveSelect::Master };
	...
	void send_command(AtaCommand command) const;
	void register_write(AtaControlRegister reg, uint8 byte) const;
	void register_write(AtaRegister reg, uint8 byte) const;
	uint8 register_read(AtaControlRegister reg) const;
	uint8 register_read(AtaRegister reg) const;
	bool identify();
	AddressingSupportSet detect_addressing();
	uint64 read_sector_count();

	bool access(uint64 base_sector, uint16 count, uint8* buf, Direction dir);
	bool access_lba28(uint32 base_sector, uint16 count, uint8* buf, Direction dir);
	bool access_lba48(uint64 base_sector, uint16 count, uint8* buf, Direction dir);
public:
	...
	constexpr uint32 sector_size() const { return m_sector_size; }
	constexpr uint64 sectors() const { return m_sectors; }
};
