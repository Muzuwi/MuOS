...
class VBitmap {
	...
	constexpr static inline int64 divround(const int64 n, const int64 d) {
	...
	inline bool bit_get(size_t idx) {
		...
		auto const& byte = *(static_cast<uint8*>(m_base) + index);
	...
	inline void bit_set(size_t idx, bool value) {
		const auto index = idx / 8;
		const auto offset = idx % 8;
		auto& byte = *(static_cast<uint8*>(m_base) + index);
...
public:
	...
	constexpr VBitmap(void* base, size_t entries)
	    : m_base(base)
	    , m_buffer_size(divround(entries, 8))
	    , m_entries(entries)
	    , m_used(0) {}

	constexpr size_t used() const { return m_used; }
	constexpr size_t entries() const { return m_entries; }
	constexpr size_t buffer_size() const { return m_buffer_size; }
