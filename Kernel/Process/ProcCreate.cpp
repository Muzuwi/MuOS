...
Process* Process::create_userland_test() {
	...
	uint8_t bytes[] = {
			0x48, 0xB8, 0x6F, 0x72, 0x6C, 0x64, 0x21, 0x00, 0x00, 0x00, 0x50, 0x48, 0xB8, 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x2C, 0x20, 0x77, 0x50, 0x48, 0x89, 0xE7, 0x48, 0xC7, 0xC0, 0xFF, 0x00, 0x00, 0x00,
			0x0f, 0x05,
			0xb8, 0x10, 0x0, 0x10, 0x0, 0xc7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x90, 0xeb, 0xfd
	};
	for(auto& b : bytes) {
		*page = b;
		page++;
...
void* Process::create_kernel_stack() {
	...
	if(m_flags.randomize_vm) {
		kstack_top = (uintptr_t)&_ukernel_virt_kstack_start
