...
struct ProcFlags {
	...
	static constexpr ProcFlags flags_for_kernel_proc() {
		return ProcFlags { .privilege = Kernel, .randomize_vm = true };
	}

	static constexpr ProcFlags flags_for_user_proc() { return ProcFlags { .privilege = User, .randomize_vm = true }; }
};
...
public:
	...
	static SharedPtr<Thread> create_with_main_thread(gen::String name, SharedPtr<Process> parent, void (*kernel_exec)(),
	                                                 ProcFlags flags = ProcFlags::flags_for_kernel_proc());
