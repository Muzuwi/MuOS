...
namespace libfdt {
	...
	void visit_each_node(FdtHeader const* header, FdtNodeVisitor visitor, void* args) {
		...
		const uint32_t off_dt_struct = be32toh(header->off_dt_struct);
		const uint32_t totalsize = be32toh(header->totalsize);
		uint8_t const* dt_struct = (uint8_t*)header + off_dt_struct;
		uint8_t const* p = dt_struct;
		uint8_t const* end = (uint8_t const*)header + totalsize;
		while(p < end) {
	...
	static inline void* _prop_data_ptr(FdtHeader const*, FdtPropHandle phandle) {
	...
	char const* node_name(FdtHeader const* header, FdtNodeHandle nhandle) {
		if(!header || !nhandle) {
			return nullptr;
		}
	...
	char const* prop_name(FdtHeader const* header, FdtPropHandle phandle) {
		...
		const uint32_t nameoff = be32toh(*(static_cast<uint32_t*>(phandle) + 1));
		const uint32_t off_dt_strings = be32toh(header->off_dt_strings);
		char const* dt_strings = (char const*)header + off_dt_strings;
		return dt_strings + nameoff;
	}

	uint32_t prop_len(FdtHeader const* header, FdtPropHandle phandle) {
		if(!header || !phandle) {
			return 0;
		}
		const uint32_t len = be32toh(*(static_cast<uint32_t*>(phandle) + 0));
		return len;
	}
	...
	char const* prop_read_string(FdtHeader const* header, FdtPropHandle phandle) {
		...
		if(prop_len(header, phandle) == 0) {
			return nullptr;
		}
		return (char const*)_prop_data_ptr(header, phandle);
	}

	bool prop_read_u32(FdtHeader const* header, FdtPropHandle phandle, uint32_t* outptr, size_t cell) {
		if(!header || !phandle || !outptr) {
			return false;
		}
		if(prop_len(header, phandle) < 4 * (cell + 1)) {
			return false;
		}

		uint32_t value;
		__builtin_memcpy(&value, _prop_data_ptr(header, phandle), 4);
		*outptr = be32toh(value);
		return true;
	}

	bool prop_read_u64(FdtHeader const* header, FdtPropHandle phandle, uint64_t* outptr, size_t cell) {
		if(!header || !phandle || !outptr) {
			return false;
		}
		if(prop_len(header, phandle) < 8 * (cell + 1)) {
			return false;
		}

		uint64_t value;
		__builtin_memcpy(&value, (char const*)_prop_data_ptr(header, phandle) + cell * 8, 8);
	...
	static bool _node_cb(FdtHeader const* header, FdtNodeHandle nhandle, void* args) {
		auto* ctx = static_cast<_FdtFindContext*>(args);
		auto* name = node_name(header, nhandle);
	...
	static bool _prop_cb(FdtHeader const* header, FdtNodeHandle, FdtPropHandle phandle, void* args) {
		auto* ctx = static_cast<_FdtFindContext*>(args);
		auto* name = prop_name(header, phandle);
	...
	int find_all_nodes_by_unit_name(FdtHeader const* header, char const* unit_name, FdtNodeHandle* out,
		...
		visit_each_node(header, _node_cb, &ctx);
	...
	FdtPropHandle node_get_named_prop(FdtHeader const* header, FdtNodeHandle nhandle, char const* prop_name) {
		...
		ctx.handle = &out;
		ctx.max_handles = 1;
		visit_each_prop(header, nhandle, _prop_cb, &ctx);
		return out;
	}
	...
	bool find_next_child(FdtHeader const* header, FdtNodeHandle parent, FdtNodeHandle* out) {
		...
		uint8_t* p;
		if(initial_node) {
			p = static_cast<uint8_t*>(initial_node);
			//  Set depth=1 to skip over the child node
			depth = 1;
		} else {
			p = static_cast<uint8_t*>(parent);
